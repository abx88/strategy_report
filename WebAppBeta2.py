...
if pagina == 'analisi Daily':
    ...
    if options==('range'):
        ...
        range_bar.update_layout( title_text="range per candela "+ticker)
        ...
        body_bar.update_layout( title_text="body per candela "+ticker)
    ...
    #ultimo mese
    if month=='1':
        lastmonth=lastyear+str(12)
    else:    
        lastmonth=year+str((df.index.max().month)-1)
    ...
    #df=imp.import_mt5("Documents/Python/Python_Analisi_Quantitativa/WebAppStream/dati_input/"+ticker+".csv")
    zig = st.slider('ampiezza swing', 0.00, 0.40, 0.05)
    df=imp.import_mt5(path+"/dati_input/"+ticker+".csv")
    df['HOD']=df.index.hour
    df['body']=df.close-df.open
    df['candela']=df.index.time
    ...
    swingdistr=px.histogram(dfswing, x="abs")
...
#st.text(df.[(df.DOWname==option)&(df.HOD>select_HOD[0])&(df.HOD<select_HOD[1])].count())
